...
PhidgetReturnCode
openServerChannel(mosiop_t iop, uint64_t pid, Phidget_ChannelClass chclass, int chidx, int persistence, PhidgetNetConnHandle nc,
	...
	if (channel == NULL) {
		...
		channel->openInfo->persistence = persistence;
	} else {
	...
	if (!PhidgetCKFlags(channel, PHIDGET_OPEN_FLAG)) {
		...
		// if persistence on the existing channel is disabled, and it's enabled by this open, enable on the channel and let clients know
		if (persistence && !PhidgetCKFlags(channel, PHIDGET_PERSISTENCE_FLAG)) {
			PhidgetSetFlags(channel, PHIDGET_PERSISTENCE_FLAG);
			bridgeSendToChannel(channel, BP_SETCHANNELPERSISTENCE, 1, "%d", 1);
		}

		// If persistence on the existing channel is enabled, and the new client didn't set persistence, let them know it's set
		if (!persistence && PhidgetCKFlags(channel, PHIDGET_PERSISTENCE_FLAG))
			bridgeSendToChannel(channel, BP_SETCHANNELPERSISTENCE, 1, "%d", 1);
	}
 ...
 * rebuild.
 */
void sendToNetworkConnections(PhidgetChannelHandle channel, BridgePacket *bp, PhidgetNetConnHandle ignoreNC) {
	PhidgetChannelNetConn *cnc;
	...
	MTAILQ_FOREACH(cnc, &channel->netconns, link) {
		 ...
		 * Note we just ignore any errors since we are sending in a loop
		 */
