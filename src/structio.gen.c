...
PhidgetReturnCode
readLEDArrayColor(BridgePacket *bp, PhidgetLEDArray_ColorHandle dst) {

	dst->r = getBridgePacketUInt8ByName(bp, "LEDArrayColor.r");
	dst->g = getBridgePacketUInt8ByName(bp, "LEDArrayColor.g");
	dst->b = getBridgePacketUInt8ByName(bp, "LEDArrayColor.b");
	dst->w = getBridgePacketUInt8ByName(bp, "LEDArrayColor.w");

	return (EPHIDGET_OK);
}

PhidgetReturnCode
writeLEDArrayColor(const PhidgetLEDArray_ColorHandle src, BridgePacket *bp) {
	PhidgetReturnCode res;

	res = addBridgePacketUInt8(bp, src->r, "LEDArrayColor.r");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketUInt8(bp, src->g, "LEDArrayColor.g");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketUInt8(bp, src->b, "LEDArrayColor.b");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketUInt8(bp, src->w, "LEDArrayColor.w");
	if (res != EPHIDGET_OK)
		return (res);
...
PhidgetReturnCode
readAnimation(BridgePacket *bp, PhidgetLEDArray_AnimationHandle dst) {

	dst->startAddress = getBridgePacketUInt32ByName(bp, "Animation.startAddress");
	dst->endAddress = getBridgePacketUInt32ByName(bp, "Animation.endAddress");
	dst->time = getBridgePacketUInt32ByName(bp, "Animation.time");
	dst->animationType = getBridgePacketInt32ByName(bp, "Animation.animationType");

	return (EPHIDGET_OK);
}

PhidgetReturnCode
writeAnimation(const PhidgetLEDArray_AnimationHandle src, BridgePacket *bp) {
	PhidgetReturnCode res;

	res = addBridgePacketUInt32(bp, src->startAddress, "Animation.startAddress");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketUInt32(bp, src->endAddress, "Animation.endAddress");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketUInt32(bp, src->time, "Animation.time");
	if (res != EPHIDGET_OK)
		return (res);

	res = addBridgePacketInt32(bp, src->animationType, "Animation.animationType");
	if (res != EPHIDGET_OK)
		return (res);
