...
#if PHIDUID_NFC0000_0_400_VINT_SUPPORTED
static PhidgetReturnCode
sendNFC0000_0_400_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {	
	assert(ch);
	assert(bp);

	return PhidgetRFIDSupport_bridgeInput(ch, bp);
}

static PhidgetReturnCode
recvNFC0000_0_400_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	int pkt;
...
sendADP0001_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_ADP0001_DATAADAPTER_100_VINT:
		...
		case BP_SETIOVOLTAGE:
			...
			if (ret == EPHIDGET_OK) {
				phid->dataAdapterVoltageDevProp = getBridgePacketInt32(bp, 0);
				bridgeSendToChannelSiblings(ch, BP_IOVOLTAGECHANGE, 1, "%d", phid->dataAdapterVoltageDevProp);
			}
			return ret;
		default:
...
recvADP0001_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_ADP0001_DATAADAPTER_100_VINT:
		...
		case VINT_PACKET_TYPE_DATAADAPTER_VOLTAGE_ERROR:
			PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OVERCURRENT, "OverCurrent Condition Detected");
			return (EPHIDGET_OK);
		default:
...
recvADP0002_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_ADP0002_DIGITALOUTPUT_100_VINT:
		...
		case VINT_PACKET_TYPE_DIGITALOUTPUT_INVALID:
			...
			return (EPHIDGET_OK);
		case VINT_PACKET_TYPE_DATAADAPTER_VOLTAGE_ERROR:
			PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OVERCURRENT, "OverCurrent Error Detected");
		default:
			MOS_PANIC("Unexpected packet type");
		}
	default:
...
sendTMP1200(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_100:
		...
		case BP_SETRTDWIRESETUP:
			...
			if (ret == EPHIDGET_OK) {
				phid->RTDWireSetupDevProp = getBridgePacketInt32(bp, 0);
				bridgeSendToChannelSiblings(ch, BP_RTDWIRESETUPCHANGE, 1, "%d", phid->RTDWireSetupDevProp);
			}
			return (ret);
		default:
			MOS_PANIC("Unexpected packet type");
...
sendRCC1000(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_RCC1000_RCSERVO_100:
		...
		case BP_SETVOLTAGE:
			buf[0] = getBridgePacketInt32(bp, 0);
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_RCSERVO_VOLTAGE, buf, 1);
			// Need to update the other channels because Voltage is board-wide
			if (ret == EPHIDGET_OK) {
				phid->RCServoVoltageDevProp = getBridgePacketInt32(bp, 0);
				bridgeSendToChannelSiblings(ch, BP_RCSERVOVOLTAGECHANGE, 1, "%d", phid->RCServoVoltageDevProp);
			}
			return (ret);
...
sendMOT0110_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_MOT0110_SPATIAL_100_VINT:
		...
		case BP_SETHEATINGENABLED:
			...
			if (ret == EPHIDGET_OK) {
				phid->heatingEnabledDevProp = getBridgePacketInt32(bp, 0);
				bridgeSendToChannelSiblings(ch, BP_HEATINGENABLEDCHANGE, 1, "%d", phid->heatingEnabledDevProp);
			}
			return (ret);
		default:
			MOS_PANIC("Unexpected packet type");
	...
	case PHIDCHUID_LED1000_DIGITALOUTPUT_100:
		...
		case BP_SETLEDFORWARDVOLTAGE:
			...
			case LED_FORWARD_VOLTAGE_5_6V:
				ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DIGITALOUTPUT_LEDFORWARDVOLTAGE, buf, 1);
				if (ret == EPHIDGET_OK) {
...
sendDAQ1400(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DAQ1400_FREQUENCYCOUNTER_130:
		...
		case BP_SETPOWERSUPPLY:
			...
			// Need to update the other channels because power supply is board-wide
			if (ret == EPHIDGET_OK) {
				phid->powerSupplyDevProp = getBridgePacketInt32(bp, 0);
				bridgeSendToChannelSiblings(ch, BP_POWERSUPPLYCHANGE, 1, "%d", phid->powerSupplyDevProp);
			}
			return (ret);
		case BP_SETINPUTMODE:
			buf[0] = getBridgePacketInt32(bp, 0);
			...
			if (ret == EPHIDGET_OK) {
				phid->inputModeDevProp = getBridgePacketInt32(bp, 0);
				bridgeSendToChannelSiblings(ch, BP_INPUTMODECHANGE, 1, "%d", phid->inputModeDevProp);
			}
			return (ret);
		default:
			MOS_PANIC("Unexpected packet type");
...
#if PHIDUID_MOT1100_OBSOLETE_SUPPORTED
static PhidgetReturnCode
sendMOT1100_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recvMOT1100_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_MOT1100_OBSOLETE_SUPPORTED */

#if PHIDUID_VINTACCEL_OBSOLETE_SUPPORTED
static PhidgetReturnCode
sendVINTACCEL_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recvVINTACCEL_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_VINTACCEL_OBSOLETE_SUPPORTED */
...
#if PHIDUID_VCP1100_1_OBSOLETE_SUPPORTED
static PhidgetReturnCode
sendVCP1100_1_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recvVCP1100_1_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_VCP1100_1_OBSOLETE_SUPPORTED */
...
#endif /* PHIDUID_CURLOOP_OBSOLETE_SUPPORTED */

#if PHIDUID_ADP1001_OBSOLETE_SUPPORTED
static PhidgetReturnCode
sendADP1001_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recvADP1001_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_ADP1001_OBSOLETE_SUPPORTED */

#if PHIDUID_1055_1_VINT_OBSOLETE_SUPPORTED
static PhidgetReturnCode
send1055_1_VINT_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recv1055_1_VINT_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_1055_1_VINT_OBSOLETE_SUPPORTED */

#if PHIDUID_DCC1010_OBSOLETE_SUPPORTED
static PhidgetReturnCode
sendDCC1010_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recvDCC1010_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_DCC1010_OBSOLETE_SUPPORTED */
