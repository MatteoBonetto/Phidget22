...
_initAfterOpen(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_1058_PHADAPTER_100:
		...
		// Inputs, Settables
		ch->dataInterval = parentPHSensor->PHSensorDataInterval[ch->phid.index];
		ch->PH = parentPHSensor->PH[ch->phid.index];
		ch->PHChangeTrigger = parentPHSensor->PHChangeTrigger[ch->phid.index];
		ch->correctionTemperature = parentPHSensor->correctionTemperature[ch->phid.index];
		break;
	case PHIDCHUID_ADP1000_PHSENSOR_100:
		...
		ch->PH = PUNK_DBL;
		ch->PHChangeTrigger = PUNK_DBL;
		ch->correctionTemperature = PUNK_DBL;
		break;
	default:
