...
PhidgetHub_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	hub = (PhidgetHubDeviceHandle)phid->parent;

	switch (bp->vpkt) {
	case BP_SETFIRMWAREUPGRADEFLAG:
		TESTRANGE_IOP(bp->iop, "%d", getBridgePacketInt32(bp, 0), 0, (hub->numVintPorts - 1));
...
PhidgetHub_getPortMode(PhidgetHubHandle ch, int port, PhidgetHub_PortMode *mode) {
	...
	if (port < 0 || port > (parentHub->numVintPorts - 1)) {
		PhidgetRelease(&parentHub);
		return (PHID_RETURN_ERRSTR(EPHIDGET_INVALIDARG, "Value must be in range: 0 - %d.", (parentHub->numVintPorts - 1)));
	}
