...
_initAfterOpen(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_1202_TEXTLCD_000:
		...
		// Inputs, Settables
		ch->backlight = parentTextLCD->backlight[ch->phid.index];
		ch->contrast = parentTextLCD->contrast[ch->phid.index];
		ch->cursorBlink = parentTextLCD->cursorBlink[ch->phid.index];
		ch->cursorOn = parentTextLCD->cursorOn[ch->phid.index];
		ch->autoFlush = 0;
		break;
	...
	case PHIDCHUID_1204_TEXTLCD_000:
		...
		ch->cursorBlink = parentTextLCD->cursorBlink[ch->phid.index];
		ch->cursorOn = parentTextLCD->cursorOn[ch->phid.index];
		ch->screenSize = parentTextLCD->screenSize[ch->phid.index];
		ch->autoFlush = 0;
		break;
	...
	case PHIDCHUID_LCD1100_LCD_100:
		...
		// Inputs, Settables
		ch->backlight = PUNK_DBL;
		ch->contrast = PUNK_DBL;
		ch->frameBuffer = PUNK_INT32;
		ch->sleeping = PUNK_BOOL;
		ch->autoFlush = 0;
		break;
