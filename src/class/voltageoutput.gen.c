...
_initAfterOpen(PhidgetChannelHandle phid) {
	PhidgetAnalogDeviceHandle parentAnalog;
	PhidgetVoltageOutputHandle ch;
	...
	parentAnalog = (PhidgetAnalogDeviceHandle)phid->parent;
	...
	case PHIDCHUID_1002_VOLTAGEOUTPUT_100:
		...
		// Inputs, Settables
		ch->enabled = parentAnalog->enabled[ch->phid.index];
		ch->voltage = parentAnalog->voltage[ch->phid.index];
		break;
	...
	case PHIDCHUID_OUT1001_VOLTAGEOUTPUT_100:
		...
		ch->voltage = PUNK_DBL;
		ch->voltageOutputRange = PUNK_ENUM;
		break;
