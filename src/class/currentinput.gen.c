...
_initAfterOpen(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_1061_CURRENTINPUT_100:
		...
		// Inputs, Settables
		ch->dataInterval = parentAdvancedServo->currentInputDataInterval[ch->phid.index];
		ch->current = parentAdvancedServo->current[ch->phid.index];
		ch->currentChangeTrigger = parentAdvancedServo->currentChangeTrigger[ch->phid.index];
		break;
	...
	case PHIDCHUID_1063_CURRENTINPUT_100:
		...
		ch->minCurrentChangeTrigger = 0;
		// Inputs, Settables
		ch->dataInterval = parentStepper->currentInputDataInterval[ch->phid.index];
		ch->current = parentStepper->current[ch->phid.index];
		ch->currentChangeTrigger = parentStepper->currentChangeTrigger[ch->phid.index];
		break;
	case PHIDCHUID_1064_CURRENTINPUT_100:
		...
		ch->minCurrentChangeTrigger = 0;
		// Inputs, Settables
		ch->dataInterval = parentMotorControl->currentInputDataInterval[ch->phid.index];
		ch->current = parentMotorControl->current[ch->phid.index];
		ch->currentChangeTrigger = parentMotorControl->currentChangeTrigger[ch->phid.index];
		break;
	...
	case PHIDCHUID_DAQ1400_CURRENTINPUT_100:
		...
		ch->current = PUNK_DBL;
		ch->currentChangeTrigger = PUNK_DBL;
