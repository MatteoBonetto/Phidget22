...
_initAfterOpen(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_1044_SPATIAL_500:
		...
		ch->dataInterval = parentSpatial->dataInterval[ch->phid.index];
		ch->precision = parentSpatial->precisionDevProp;
		ch->algorithm = parentSpatial->algorithm[ch->phid.index];
		ch->algorithmMagnetometerGain = parentSpatial->algorithmMagnetometerGain[ch->phid.index];
		break;
	...
	case PHIDCHUID_MOT1102_SPATIAL_200:
		...
		// Inputs, Settables
		ch->dataInterval = PUNK_UINT32;
		ch->algorithm = PUNK_ENUM;
		ch->algorithmMagnetometerGain = PUNK_DBL;
		break;
