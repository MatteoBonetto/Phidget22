...
_initAfterOpen(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_1008_ACCELEROMETER_000:
		...
		ch->axisCount = 2;
		// Inputs, Settables
		ch->dataInterval = parentAccelerometer->dataInterval[ch->phid.index];
		...
		ch->accelerationChangeTrigger = parentAccelerometer->accelerationChangeTrigger[ch->phid.index];
		break;
	case PHIDCHUID_1041_ACCELEROMETER_200:
		...
		// Inputs, Settables
		ch->dataInterval = parentSpatial->dataInterval[ch->phid.index];
		...
		ch->accelerationChangeTrigger = parentSpatial->accelerationChangeTrigger[ch->phid.index];
		break;
	...
	case PHIDCHUID_1043_ACCELEROMETER_300:
		...
		ch->acceleration[2] = parentSpatial->acceleration[ch->phid.index][2];
		ch->accelerationChangeTrigger = parentSpatial->accelerationChangeTrigger[ch->phid.index];
		ch->precision = parentSpatial->precisionDevProp;
		break;
	...
	case PHIDCHUID_MOT1100_ACCELEROMETER_200:
		...
		ch->accelerationChangeTrigger = PUNK_DBL;
		break;
	...
	case PHIDCHUID_MOT0109_ACCELEROMETER_100:
		...
		ch->accelerationChangeTrigger = parentSpatial->accelerationChangeTrigger[ch->phid.index];
		ch->precision = parentSpatial->precisionDevProp;
		ch->heatingEnabled = parentSpatial->heatingEnabledDevProp;
		break;
	...
	case PHIDCHUID_MOT0110_ACCELEROMETER_100_VINT:
		...
		ch->acceleration[2] = PUNK_DBL;
		ch->accelerationChangeTrigger = PUNK_DBL;
		ch->heatingEnabled = parentVINT->heatingEnabledDevProp;
		break;
	default:
...
_setDefaults(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_MOT0109_ACCELEROMETER_100:
		...
		if (ch->precision == PUNK_ENUM)
			ch->precision = SPATIAL_PRECISION_HYBRID;
		// (Device Property - check if already set)
		if (ch->heatingEnabled == PUNK_BOOL)
			ch->heatingEnabled = 0;
		// Must-set values
		// Default values
		ch->dataInterval = 256;
