...
_initAfterOpen(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_1015_CAPACITIVETOUCH_000:
		...
		ch->minTouchValue = 0;
		// Inputs, Settables
		ch->touchValueChangeTrigger = parentInterfaceKit->touchValueChangeTrigger[ch->phid.index];
		ch->dataInterval = parentInterfaceKit->capacitiveTouchDataInterval[ch->phid.index];
	...
	case PHIDCHUID_HIN1000_CAPACITIVETOUCH_100:
		...
		ch->minTouchValue = 0;
		// Inputs, Settables
		ch->touchValueChangeTrigger = PUNK_DBL;
		ch->dataInterval = PUNK_UINT32;
		ch->sensitivity = PUNK_DBL;
