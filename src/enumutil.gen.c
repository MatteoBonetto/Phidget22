...
Phidget_enumFromString(const char *name) {
	...
	if (mos_strcasecmp(name, "EEPHIDGET_BADCURRENT") == 0)
		return (4118);
	...
	if (mos_strcasecmp(name, "LED_COLOR_ORDER_RGB") == 0)
		return (1);

	if (mos_strcasecmp(name, "LED_COLOR_ORDER_GRB") == 0)
		return (2);

	if (mos_strcasecmp(name, "LED_COLOR_ORDER_RGBW") == 0)
		return (3);

	if (mos_strcasecmp(name, "LED_COLOR_ORDER_GRBW") == 0)
		return (4);
	...
	if (mos_strcasecmp(name, "ANIMATION_TYPE_RANDOMIZE") == 0)
		return (3);
...
Phidget_enumString(const char *family, int id) {
	...
	if (mos_strcasecmp(family, "LEDArrayColorOrder") == 0) {
		switch (id) {
		case 1:
...
supportedUnit(PhidgetChannelHandle ch, Phidget_Unit val) {
	...
	case PHIDCHUID_ifkit488_VOLTAGERATIOINPUT_000:
		switch (val) {
		case PHIDUNIT_NONE:
		case PHIDUNIT_BOOLEAN:
		case PHIDUNIT_PERCENT:
		case PHIDUNIT_DECIBEL:
		case PHIDUNIT_MILLIMETER:
		case PHIDUNIT_CENTIMETER:
		case PHIDUNIT_METER:
		case PHIDUNIT_GRAM:
		case PHIDUNIT_KILOGRAM:
		case PHIDUNIT_MILLIAMPERE:
		case PHIDUNIT_AMPERE:
		case PHIDUNIT_KILOPASCAL:
		case PHIDUNIT_VOLT:
		case PHIDUNIT_DEGREE_CELCIUS:
		case PHIDUNIT_LUX:
		case PHIDUNIT_GAUSS:
		case PHIDUNIT_PH:
		case PHIDUNIT_WATT:
...
int
supportedLEDArrayColorOrder(PhidgetChannelHandle ch, PhidgetLEDArray_ColorOrder val) {
