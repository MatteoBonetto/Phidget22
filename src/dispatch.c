...
dispatchEntry(PhidgetHandle phid, DispatchEntryHandle de) {
	...
	if (channel) {
		...
		case DE_CHANNEL_INITEVENTS:
			...
			break;
		case DE_CHANNEL_SETSTATUS: {
			PhidgetReturnCode bpres;

			mos_mutex_lock(&channel->netconnslk);
			bpres = channel->getStatus(channel, &bp);
		...
		setstatus_again:
			MTAILQ_FOREACH(cnc, &channel->netconns, link) {
				...
				if (bpres != EPHIDGET_OK) {
					// NOTE: This failure should not be possible without a library bug
					nc = cnc->nc;
					...
					// send the reply after closing the channel
					sendSimpleReply(nc, repseq, bpres, "getStatus() failure.");
					// closeServerChannel removes this network connection from the channel, so we have to break out of the loop
					goto setstatus_again;
				}
				...
				// We ignore any errors since we are in a loop anyway
				networkSendBridgePacket(channel, bp, cnc->nc);
			}
			destroyBridgePacket(&bp);
		...
		case DE_CHANNEL_ATTACH: {
			...
			if (!isNetworkPhidget(channel)) {
				// NOTE: We let the RESET packet through to vintdevice.c so the port mode can be set.
				//  It blocks the RESET message to the device if needed.
				if (isVintChannel(channel) || !PhidgetCKFlags(channel, PHIDGET_PERSISTENCE_FLAG)) {
					...
					if (res != EPHIDGET_OK) {
						logerr("Failed to send BP_OPENRESET to %" PRIphid ": " PRC_FMT, channel, PRC_ARGS(res));
						mos_snprintf(errbuf, 256, "Failed to open channel: " PRC_FMT, PRC_ARGS(res));
						goto attach_error;
					}
				}
				...
				if (res != EPHIDGET_OK) {
					...
					mos_snprintf(errbuf, 256, "Failed to initialize channel: " PRC_FMT, PRC_ARGS(res));
					goto attach_error;
				}

				// Defaults must be set by the user in persistence mode
				if (!PhidgetCKFlags(channel, PHIDGET_PERSISTENCE_FLAG)) {
			...
			if (!isNetworkPhidget(channel)) {
				...
				if (res != EPHIDGET_OK) {
					logerr("Failed to send BP_ENABLE to %" PRIphid ": " PRC_FMT, channel, PRC_ARGS(res));
					mos_snprintf(errbuf, 256, "Failed to enable channel: " PRC_FMT, PRC_ARGS(res));
		...
		attach_error:
			...
			// Only for local attach
			if (!isNetworkPhidget(channel)) {
				// Notify any client waiting on attach to finish. (ie local attach in the network server context)
				mos_mutex_lock(&channel->netconnslk);
			attach_error_servers_again:
				MTAILQ_FOREACH(cnc, &channel->netconns, link) {
					if (cnc->setstatusrep == 0)
						continue;
					repseq = cnc->setstatusrep;
					...
					// send the reply after closing the channel
					sendSimpleReply(nc, repseq, res, errbuf);
					// closeServerChannel removes this network connection from the channel, so we have to break out of the loop
					goto attach_error_servers_again;
				}
				mos_mutex_unlock(&channel->netconnslk);
			}
